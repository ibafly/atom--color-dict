...
export default {
  ...
  getColorCode: function(span) {
    ...
    nameWordArray.shift();
    console.log(nameWordArray);
    nameWordArray = nameWordArray.join('').toLowerCase();
    console.log(nameWordArray);
    let matchedItem = colorNameList.find(item => {
      return deburr(item.name).replace(reg, '').toLowerCase() === nameWordArray ? item : null;
    });
  ...
  getNameVars: function(line) {
    ...
    if (colorIndicator) {
      ...
      if (nameVarsArray = line.match(INDI_VARS)) {
        let i = -1;
        console.log(line);
        console.log(nameVarsArray);
        console.log(nameVarsArray.length);
