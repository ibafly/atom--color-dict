...
function matchFromSymbolsRangeToHex (symbolObj) {
  // let reg = /['\u2019\s]/g
  const reg = /[^A-Za-z0-9]/g
  const regCo2 = /₂/g
  let namesWordsArray = words(symbolObj.nameVar)
...
function matchSymbolsInOneLine (line, indicator) {
  let symbolsArray = []
  let matched = []
  if (indicator) {
...
function getNewRangesBehindCursors (cursors, ranges) {
  ranges.filter((range) => {
